...
function generateNumbers() {
    ...
    if (isNaN(fixed1) || (step >= 2 && isNaN(fixed2)) || (step >= 3 && isNaN(fixed3)) || (step >= 4 && isNaN(fixed4))) {
    ...
    const fixedNumbers = [fixed1, fixed2, fixed3, fixed4].filter(num => !isNaN(num));
    const allNumbers = Array.from({ length: 10 }, (_, i) => i);
    const availableNumbers = allNumbers.filter(num => !fixedNumbers.includes(num));
    const randomNumbers = [];

    while (randomNumbers.length < (7 - fixedNumbers.length)) {
...
function exportToExcel() {
    ...
    XLSX.writeFile(workbook, "jogos_super_sete.xlsx");
}
